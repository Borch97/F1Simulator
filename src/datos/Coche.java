...
public class Coche {
	...
	/**
	 * Metodo para realizar la suma total de tiempo de cada piloto
	 * @param mins
	 * @param segs
	 * @param msegs
	 */
	public void incrementarTiempo(int mins, int segs, int msegs) {
		if (tiempo != null) {
			long t1 = Simulacion.s.rangoMilisegundos(tiempo.getMinutes(), tiempo.getSeconds(),
					tiempo.getMilliseconds());
	...
	public Coche(String nom_piloto, String abreviado, String escuderia, double velocidad, double aceleracion,
			double aerodinamica, double prob_rotura, double neumaticos, Image imagen, int paradasBoxes,
			String nom_usuario, int puntos) {
		this.nom_piloto = nom_piloto;
	 ...
	 * 
	 * public void setNom_usuario(String nom_usuario) { this.nom_usuario =
	 * nom_usuario; }
	 */
