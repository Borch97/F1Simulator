...
public class Gestion {
	...
	/**
	 * Declaracion de parametros y arrays que se van a necesitar
	 */
	...
	public ArrayList<String> arrayNombres; // = new
											// ArrayList<>(Arrays.asList("Hamilton",
											// "Vettel", "Verstappen", "Alonso",
											// "Button", "Sainz", "Massa",
											// "Rosberg", "Ricciardo", "Perez",
											// "Bottas"));
	public ArrayList<String> arrayEscuderias; // = new
												// ArrayList<>(Arrays.asList("Mercedes",
												// "Ferrari", "Williams",
	...
	/**
	 * Cargar arrays con la informacion de la BD
	 */
	public Gestion() {
	...
	/**
	 * Metodo que busca el nombre de los pilotos
	 * 
	 * @param arrayCoche
	 * @param nombre
	 * @return
	...
	/**
	 * Metodo que dado un rango de valores, te devuelve un numero aleatorio
	 * entre dichos valores
	 * 
	 * @param min
	 *            Valor entero minimo desde el cual deseas crear un numero
	 *            aleatorio
	 * @param max
	 *            Valor entero maximo hasta el cual deseas crear un numero
	 *            aleatorio
	 * @return Devuelve un valor entero aleatorio dentro del rango establecido
	...
	/**
	 * Metodo al que se llama para crear una partida nueva
	 */
	...
	/**
	 * Metodo para obtener la posicion de los pilotos
	 * 
	 * @return
	 */
	...
	/**
	 * Metodo para obtener la posicion del usuario
	 * 
	 * @return
	 */
	...
	/**
	 * Metodo para dar dinero a los pilotos
	 */
	public void agregarDinero() {
		switch (obtenerPosicion()) {
		case 0:
			arrayUsuario.get(obtenerPosicionUsuario())
					.setDinero(arrayUsuario.get(obtenerPosicionUsuario()).getDinero() + 110000);
			break;
		case 1:
			arrayUsuario.get(obtenerPosicionUsuario())
					.setDinero(arrayUsuario.get(obtenerPosicionUsuario()).getDinero() + 100000);
			break;
		case 2:
			arrayUsuario.get(obtenerPosicionUsuario())
					.setDinero(arrayUsuario.get(obtenerPosicionUsuario()).getDinero() + 90000);
			break;
		case 3:
			arrayUsuario.get(obtenerPosicionUsuario())
					.setDinero(arrayUsuario.get(obtenerPosicionUsuario()).getDinero() + 80000);
			break;
		case 4:
			arrayUsuario.get(obtenerPosicionUsuario())
					.setDinero(arrayUsuario.get(obtenerPosicionUsuario()).getDinero() + 70000);
			break;
		case 5:
			arrayUsuario.get(obtenerPosicionUsuario())
					.setDinero(arrayUsuario.get(obtenerPosicionUsuario()).getDinero() + 60000);
			break;
		case 6:
			arrayUsuario.get(obtenerPosicionUsuario())
					.setDinero(arrayUsuario.get(obtenerPosicionUsuario()).getDinero() + 50000);
			break;
		case 7:
			arrayUsuario.get(obtenerPosicionUsuario())
					.setDinero(arrayUsuario.get(obtenerPosicionUsuario()).getDinero() + 40000);
			break;
		case 8:
			arrayUsuario.get(obtenerPosicionUsuario())
					.setDinero(arrayUsuario.get(obtenerPosicionUsuario()).getDinero() + 30000);
			break;
		case 9:
			arrayUsuario.get(obtenerPosicionUsuario())
					.setDinero(arrayUsuario.get(obtenerPosicionUsuario()).getDinero() + 20000);
			break;
		case 10:
			arrayUsuario.get(obtenerPosicionUsuario())
					.setDinero(arrayUsuario.get(obtenerPosicionUsuario()).getDinero() + 10000);
			break;
		default:
			break;
		}
	...
	/**
	 * Crea un nuevo piloto de tipo Coche con todos los valores aleatorios,
	 * incluidos el nombre y la escuderia, basandose en un array de Strings
	 * 
	 * @return Devuelve un nuevo piloto de tipo Coche con todos los valores
	 *         aleatorios
	 */
	public Coche creacionPiloto() {
		...
		// return new Coche(arrayNombres.get(aleatorio(0, arrayNombres.size() -
		// 1))," ", arrayEscuderias.get(aleatorio(0, arrayEscuderias.size() -
		// 1)), aleatorio(0, 10), aleatorio(0, 10), aleatorio(0, 10), 0, 100);
	}
	...
	public void creacionUsuario() {
		Usuario u = new Usuario(variableUsuario, variableUsuario.substring(0, 4),
				arrayEscuderias.get(aleatorio(0, arrayEscuderias.size() - 1)), 0, 0, 0, 0, 100, null, 0,
				variableUsuario, 0, 2000000);
		arrayUsuario.add(u);
		GestorBD.getInstance().guardarDatosUsuario(u);
	}

	/**
	 * Metodo para crear el coche del usuario
	 */
	public void creacionUsuarioCoche() {
		Coche c = new Coche(variableUsuario, variableUsuario.substring(0, 4),
				arrayEscuderias.get(aleatorio(0, arrayEscuderias.size() - 1)), 0, 0, 0, 0, 100, null, 0,
				variableUsuario, 0);
		arrayCoche.add(c);
	...
	/**
	 * Metodo para reiniciar el tiempo
	 */
	...
	/**
	 * Metodo para guardar los coches en la BD
	 */
	...
	/**
	 * Metodo para asignar los puntos conseguidos en la carrera
	 */
	...
	// Metodo que se encarga de crear automaticamente todos los otros pilotos
	// del modo carrera

	/**
	 * Metodo que crea todos los coches de la AI del juego, comprobando que que
	 * no se repita el nombre y haya un maximo de dos pilotos por escuderia Crea
	 * un numero de pilotos igual a la variable totalPilotos.
	 */
	public void creacionAI() {
		...
		if (arrayCoche.isEmpty() || arrayCoche.get(0).getNom_piloto().equals(variableUsuario)) {
			for (int j = totalPilotos - 1; j > 0; j--) {
				...
				while ((containsElement(arrayCoche, cocheComprobar.getNom_piloto()))
	...
	// Metodo que comprueba si existe otro elemento dentro del array con el
	// mismo nombre
	...
	public void copiarArray(ArrayList<Rango> arrayTiempoVueltaSoloInicial,
			ArrayList<Rango> arrayTiempoVueltaSoloCopia) {
	...
	public void copiarArrayString(ArrayList<String> arrayTiempoVueltaSoloInicial,
			ArrayList<String> arrayTiempoVueltaSoloCopia) {
		for (int i = 0; i < arrayTiempoVueltaSoloInicial.size(); i++) {
			arrayTiempoVueltaSoloCopia.add(arrayTiempoVueltaSoloInicial.get(i));
		}
		arrayTiempoVueltaSoloInicial.clear();
	...
	/**
	 * Metodo para ordenar por puntos a los pilotos
	 */
	...
	public void ordenarPorPuntos(ArrayList<Coche> arrayCocheClasificacion) {
		...
		for (int i = 0; i < arrayCocheClasificacion.size() - 1; i++) {
			...
			for (int indice = i + 1; indice < arrayCocheClasificacion.size(); indice++) {
				if (arrayCocheClasificacion.get(indice)
						.getPuntos() == (arrayCocheClasificacion.get(masGrande).getPuntos()))
					break;
				if (arrayCocheClasificacion.get(indice).getPuntos() > arrayCocheClasificacion.get(masGrande)
						.getPuntos())
					masGrande = indice;
			}

			intercambiar(i, masGrande, arrayCocheClasificacion); // intercambia
																	// el
																	// elemento
																	// m�s
																	// peque�o
	...
	public void ordenarPorTiempoTotal(ArrayList<Rango> arrayTiempoVueltaSoloInicial,
			ArrayList<String> arrayTiempoVuelta, ArrayList<Coche> arrayCoche,
			ArrayList<Rango> arrayDiferenciaTiempoVuelta) {
		...
		for (int i = 0; i < arrayCoche.size() - 1; i++) {
			...
			for (int indice = i + 1; indice < arrayCoche.size(); indice++) {
				if (arrayCoche.get(indice).getTiempo().equals(arrayCoche.get(masPequenio).getTiempo()))
					break;
				if (arrayCoche.get(indice).getTiempo().getMinutes() < arrayCoche.get(masPequenio).getTiempo()
						.getMinutes())
					masPequenio = indice;
				else if (((arrayCoche.get(indice).getTiempo()
						.getMinutes() == (arrayCoche.get(masPequenio).getTiempo().getMinutes()))
						&& (arrayCoche.get(indice).getTiempo().getSeconds() < arrayCoche.get(masPequenio).getTiempo()
								.getSeconds())))
					masPequenio = indice;
				else if (((arrayCoche.get(indice).getTiempo()
						.getMinutes() == (arrayCoche.get(masPequenio).getTiempo().getMinutes()))
						&& (arrayCoche.get(indice).getTiempo()
								.getSeconds() == (arrayCoche.get(masPequenio).getTiempo().getSeconds()))
						&& (arrayCoche.get(indice).getTiempo().getMilliseconds() < arrayCoche.get(masPequenio)
								.getTiempo().getMilliseconds())))
					masPequenio = indice;
			}

			intercambiar(i, masPequenio, arrayTiempoVueltaSoloInicial, arrayTiempoVuelta, arrayCoche,
					arrayDiferenciaTiempoVuelta); // intercambia el elemento
													// m�s peque�o en la
													// posici�n
		}
	}// fin de for exterior
	...
	public void intercambiar(int primero, int segundo, ArrayList<Rango> arrayTiempoVueltaSoloInicial,
			ArrayList<String> arrayTiempoVuelta, ArrayList<Coche> arrayCoche, ArrayList<Rango> arrayDiferenciaTiempo) {
		Rango temporal = arrayTiempoVueltaSoloInicial.get(primero); // almacena
																	// primero
																	// en
																	// temporal
		...
		arrayTiempoVueltaSoloInicial.set(primero, arrayTiempoVueltaSoloInicial.get(segundo)); // sustituye
																								// primero
																								// con
																								// segundo
		...
		arrayTiempoVueltaSoloInicial.set(segundo, temporal); // coloca temporal
																// en segundo
	...
	public void reordenarIndices(ArrayList<String> arrayTiempoVuelta) {
		for (int i = 0; i < arrayTiempoVuelta.size(); i++) {
			arrayTiempoVuelta.set(i, arrayTiempoVuelta.get(i).replaceFirst(arrayTiempoVuelta.get(i).substring(0, 1),
					Integer.toString(i + 1)));
		}
	}

	public void stringArrayOrdenado(ArrayList<Rango> arrayList, ArrayList<String> arrayString,
			ArrayList<String> posNombrePiloto) {
		int cont = 0;
		arrayString.clear();
		for (Rango r : arrayList) {
			// TODO
			arrayString.add((cont + 1) + ".-" + posNombrePiloto.get(cont).substring(0, 3) + " = " + r.getMinutes() + ":"
					+ r.getSeconds() + "," + r.getMilliseconds());
			cont++;
		}
	}
	...
	public void mejoraIAExponencial(ArrayList<Coche> arrayCoche, double porcentajeMejora) {
		for (int i = 0; i < arrayCoche.size(); i++) {
			...
			switch (a) {
			case 0:
				if (arrayCoche.get(i).getAceleracion() < 10
						&& arrayCoche.get(i).getAceleracion() * porcentajeMejora <= 10) {
				...
				break;
			case 1:
				if (arrayCoche.get(i).getAerodinamica() < 10
						&& arrayCoche.get(i).getAerodinamica() * porcentajeMejora <= 10) {
				...
				break;
			case 2:
				if (arrayCoche.get(i).getVelocidad() < 10
						&& arrayCoche.get(i).getVelocidad() * porcentajeMejora <= 10) {
	...
	public static void main(String[] args) {
		...
		// Circuito m = new Circuito("Melbourne", "Australia", 30, 3000.5, 0.0,
		// path , duration);
		// g.arrayCircuito.add(m);
		g.creacionAI();
		for (Coche coche : g.arrayCoche) {
			System.out.println("Nombre: " + coche.getNom_piloto() + " Escuderia: " + coche.getEscuderia()
					+ " Velocidad : " + coche.getVelocidad() + " Aceleracion : " + coche.getAceleracion()
					+ " Aerodinamica: " + coche.getAerodinamica() + " Probabilidad Rotura: " + coche.getProb_rotura());
		}
